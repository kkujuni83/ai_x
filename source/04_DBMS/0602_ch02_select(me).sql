...
SELECT ENAME, SAL FROM EMP WHERE ENAME BETWEEN 'C' AND 'PZ' -- P로 시작하는 이름과 P보다 사전 순으로 뒤에 오는 이름까지 포함
ORDER BY ENAME;

SELECT ENAME, SAL FROM EMP WHERE REGEXP_LIKE(ENAME, '^[C-P]') -- Oracle 
ORDER BY ENAME;

SELECT ENAME, SAL FROM EMP
WHERE ENAME LIKE 'C%' OR ENAME LIKE 'D%' OR ENAME LIKE 'E%' OR ENAME LIKE 'F%' OR ENAME LIKE 'G%'
   OR ENAME LIKE 'H%' OR ENAME LIKE 'I%' OR ENAME LIKE 'J%' OR ENAME LIKE 'K%' OR ENAME LIKE 'L%'
   OR ENAME LIKE 'M%' OR ENAME LIKE 'N%' OR ENAME LIKE 'O%' OR ENAME LIKE 'P%'
ORDER BY ENAME;

--9.	comm이 sal보다 10%가 많은 모든 사원에 대하여 이름, 급여, 상여금을 
--출력하는 SELECT 문을 작성
SELECT ENAME, SAL, COMM FROM EMP WHERE NVL(COMM, 0) > (SAL * 1.1); -- COMM이 NULL일 경우 0으로 처리하여 비교
...
-- 업무, 급여를 검색하시오.
SELECT EMPNO, ENAME, HIREDATE, JOB, SAL FROM EMP WHERE TO_CHAR(HIREDATE, 'YYYY') = '1981' AND JOB != 'SALESMAN';
...
SELECT EMPNO, ENAME, (SAL * 12) AS ANNUAL_SALARY FROM EMP WHERE (SAL * 12) >= 35000;
