...
select pno, pname, dno from personal where dno = 10;
...
-- 7. 사번, 이름, 상사이름
select e.pno, e.pname, m.pname as manager_name
from personal e
left join personal m on e.mgr = m.pno;

-- 8. 사번, 이름, 상사이름(상사가 없는 사람도 출력하되 상사가 없는 경우 ★CEO★로 출력) 
select e.pno, e.pname, ifnull(m.pname, 'CEO') as manager_name
from personal e
left join personal m on e.mgr = m.pno;

-- 8-1 사번, 이름, 상사사번(상사가 없으면 ceo로 출력. ifnull함수의 매개변수의 타입이 상이해도 상관없음)
select pno, pname, ifnull(cast(mgr as char), 'CEO') as manager_no
from personal;

-- 8-2. 사번, 이름, 상사이름, 부서명(상사가 없는 사람도 출력) – 같이 합시다
select e.pno, e.pname, 
       ifnull(m.pname, 'CEO') as manager_name, 
       d.dname
from personal e
left join personal m on e.mgr = m.pno
left join division d on e.dno = d.dno;

-- 9. 이름이 s로 시작하는 사원 이름 (like 이용)
select * from personal where pname like 's%';

-- 10. 사번, 이름, 급여, 부서명, 상사이름
select e.pno, e.pname, e.pay, d.dname, ifnull(m.pname, 'CEO') as manager_name
from personal e
